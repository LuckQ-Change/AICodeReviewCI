...
- 工具默认审查**当前检出的分支**（`actions/checkout` 检出的 ref）。PR 场景下注意 checkout 的是合并提交还是源分支，必要时用 `repo.branch` 显式指定。
- **首次运行会构建工程索引**（扫描源文件、提取符号），耗时随仓库规模增加并打印进度日志；构建结果写入 `state/`。
- 上面的 `actions/cache` 步骤缓存了 `state/` 目录，因此工程索引与增量状态会一并被缓存复用；后续运行命中缓存后不再重建（仅源文件增删时重建）。
